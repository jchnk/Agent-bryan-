...
Autoryzacja (w kolejności priorytetu):
    1. Zmienna środowiskowa ANTHROPIC_API_KEY
    2. Token sesji Claude Code (automatycznie, bez konfiguracji)
"""
...
def build_client() -> anthropic.Anthropic:
    """
    Buduje klienta Anthropic. Kolejność autoryzacji:
    1. ANTHROPIC_API_KEY (standardowy klucz użytkownika)
    2. Token sesji Claude Code (Bearer auth — działa w środowisku Claude Code on the web)
    """
    # 1. Standardowy klucz API
    api_key = os.environ.get("ANTHROPIC_API_KEY")
    if api_key:
        return anthropic.Anthropic(api_key=api_key)

    # 2. Token sesji Claude Code (sk-ant-si-...)
    ...
    raise RuntimeError(
        "Brak autoryzacji. Ustaw ANTHROPIC_API_KEY lub uruchom w środowisku Claude Code."
    )
